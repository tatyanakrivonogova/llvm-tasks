...
static constexpr char PassName[] = "custom-opts-brackets-mul-transform";
static constexpr char PassDescription[] = "Collapses brackets multiplication";

} // namespace custom_opts

llvm::PreservedAnalyses custom_opts::BracketsMulTransform::run(
...
bool custom_opts::BracketsMulTransform::run(
  const custom_opts::BracketsMultiplications& bracketsMultiplications, llvm::Function& func)
{
  bool modified{false};
  ...
  for (llvm::BinaryOperator* binOp : bracketsMultiplications.multiplications())
  {
